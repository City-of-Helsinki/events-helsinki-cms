...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        shutil.copy2(
            "pictures/gerome-bruneau-RPmWEtZLh7U-unsplash.jpg",
            "media-files/collection_hero_image.jpg",
        )
        collection_hero_image = models.CustomImage(
            title="Collection Hero Image",
            file="collection_hero_image.jpg",
            photographer_credit_fi="gerome-bruneau",
        )
        collection_hero_image.save()

        shutil.copy2(
            "pictures/davisco-5E5N49RWtbA-unsplash.jpg",
            "media-files/landing_page_hero_background_image.jpg",
        )
        hero_background_image = models.CustomImage(
            title="Landing Page Hero Background Image",
            file="landing_page_hero_background_image.jpg",
            photographer_credit_fi="davisco",
        )
        hero_background_image.save()

        shutil.copy2(
            "pictures/ravi-sharma-xxGyLaY4v14-unsplash.jpg",
            "media-files/landing_page_hero_background_image_mobile.jpg",
        )
        hero_background_image_mobile = models.CustomImage(
            title="Landing Page Hero Background Image Mobile",
            file="landing_page_hero_background_image_mobile.jpg",
            photographer_credit_fi="ravi-sharma",
        )
        hero_background_image_mobile.save()

        shutil.copy2(
            "pictures/joanna-kosinska-1_CMoFsPfso-unsplash.jpg",
            "media-files/landing_page_hero_top_layer_image.jpg",
        )
        hero_top_layer_image = models.CustomImage(
            title="Landing Page Hero Top Layer Image",
            file="landing_page_hero_top_layer_image.jpg",
            photographer_credit_fi="joanna-kosinska",
        )
        hero_top_layer_image.save()

        shutil.copy2(
            "pictures/merakist-CNbRsQj8mHQ-unsplash.jpg",
            "media-files/landing_page_social_media_image.jpg",
        )
        social_media_image = models.CustomImage(
            title="Landing Page Social Media Image",
            file="landing_page_social_media_image.jpg",
            photographer_credit_fi="merakist",
        )
        social_media_image.save()

        root_page = wagtail_models.Page.objects.get(title="Root")

        helsinki_activities = root_page.add_child(
            instance=models.HelsinkiActivities(title="Helsinki Activities")
        )

        collections_folder = helsinki_activities.add_child(
            instance=models.CollectionsFolder(title="Collections")
        )
        landing_pages_folder = helsinki_activities.add_child(
            instance=models.LandingPagesFolder(title="Landing Pages")
        )
        static_pages_folder = helsinki_activities.add_child(
            instance=models.StaticPagesFolder(title="Static Pages")
        )
        banner_pages_folder = helsinki_activities.add_child(
            instance=models.BannerPagesFolder(title="Banner Pages")
        )

        top_banner = banner_pages_folder.add_child(
            instance=models.BannerPages(
                title="Summer banner top",
                **dict(
                    BANNER_PAGE_BASE,
        ...
        bottom_banner = banner_pages_folder.add_child(
            instance=models.BannerPages(
                title="Summer banner bottom",
                **dict(
                    BANNER_PAGE_BASE,
                    hero_background_image_fi=hero_background_image,
                    hero_background_image_mobile_fi=hero_background_image_mobile,
        ...
        collections_folder.add_child(
            instance=models.Collections(
                title="Kool Kids of Kallio",
                **dict(COLLECTION_BASE, hero_image=collection_hero_image)
            )
        )

        collections_folder.add_child(
            instance=models.Collections(
                title="Kool Kids of Kamppi",
                **dict(COLLECTION_BASE, visible_on_frontpage=False)
            )
        )

        collections_folder.add_child(
            instance=models.Collections(title="Kool Kids of Kurvi", **COLLECTION_BASE)
        )

        landing_pages_folder.add_child(
            instance=models.LandingPages(
        ...
        landing_pages_folder.add_child(
            instance=models.LandingPages(title="Fall is here!", **LANDING_PAGE_BASE)
        )

        landing_pages_folder.add_child(
            instance=models.LandingPages(title="Winter is here!", **LANDING_PAGE_BASE)
        )

        static_pages_folder.add_child(
            instance=models.AboutPage(title="About", **ABOUT_PAGE_BASE)
        )
        static_pages_folder.add_child(
            instance=models.AccessibilityPage(
                title="Accessibility", **ACCESSIBILITY_PAGE_BASE
            )
        )

        # Creating a default "Site"
        default_site = wagtail_models.Site(
            hostname="localhost",
            port="8080",
            root_page=helsinki_activities,
